...
## ADR-012 — Drum-record-mode AND the chromatic sampler (per-key/shared keybed sampler) are not ported
Decision: looper's drum-record-mode (`apcKey25.cpp:112,159-165,203,219`: holding note 66 arms `m_drumRecordMode`, then each keybed key records into its OWN independent sample slot) is NOT reproduced in aloop. This is a deliberate, documented model difference, not a silent gap. CORRECTION (found via cross-codebase research after a user report of "sampler didn't work" distinct from "drum sampler didn't work"): this ADR's original scope statement undersold the gap — looper ALSO has a separate CHROMATIC sampler (note 65 held records ONE shared sample, `apcKey25.cpp:117-121,160-163,198-201`: `pSampler->chromaticLoaded()` gates whether a keybed key plays that sample pitched-per-key instead of falling through to live-pitch transpose) which is EQUALLY unported — there is no `sampler`/`drum`/note-65/note-66 reference anywhere in `src/control/apc_grid.h/.cpp` or `config/controls.conf`. The original "every other gesture maps 1:1" consequence line below was inaccurate; both sampler features (chromatic AND drum-record) are out of scope, not just drum-record-mode.
...
Consequence: the keybed's 25 pad-adjacent note range that looper repurposes for drum-record-mode AND note 65 (chromatic-sample record) are unbound in `config/controls.conf` (available for remapping); no sampler feature of any kind exists. Every other APC Key25 gesture (grid, presets, tap/hold, live-pitch INCLUDING the keybed's own live-pitch-engage fallback when no sampler content exists, microrepeat, SHIFT-gated formant range + monitor-fold + STOP_IMMEDIATE) maps 1:1 (docs/COMMAND-SURFACE.md).
